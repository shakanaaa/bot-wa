// FINAL ALL DIRECT DOWNLOAD - RELIABLE APIS
...
app.get('/download', async (req, res) => {
    ...
    try {
        ...
        // --- X / TWITTER (WORKING) ---
            ...
            try {
                // Use allorigins proxy to get Twitter video
                const tweetId = url.split('/status/')[1]?.split('?')[0];
                if (tweetId) {
                    const apiRes = await axios.get(`https://api.allorigins.win/raw?url=https://twitter.com/i/videos/tweet/${tweetId}`);
        ...
        // --- FACEBOOK (WORKING) ---
            ...
            try {
                // Use fdownloader API
                const apiRes = await axios.get(`https://fdownloader.net/api?url=${encodeURIComponent(url)}`);
